...
func _input(event):
	if (event is InputEventMouseButton) and canSelectDirection and canMove:
		...
		if pos.x < self.position.x:
			tween.tween_property($AnimatedSprite2D, "flip_h", true, 1) # flip the sprite
		else:
			tween.tween_property($AnimatedSprite2D, "flip_h", false, 1) # dont flip the sprite
...
func _on_wander_timeout():
	if !playerHasSelectedDirection and canMove:
		...
		match(randomDirection):
			0: # left
				tween.tween_property($AnimatedSprite2D, "flip_h", true, 0.5) # flip the sprite
			...
			1: #right
				tween.tween_property($AnimatedSprite2D, "flip_h", false, 0.5)
				tween.tween_property(self, "position", Vector2(pos.x + distance, pos.y), 2).set_ease(Tween.EASE_IN_OUT)
